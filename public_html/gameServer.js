  ...
  if(req.files) {
    ...
    reqImgFile.mv('./' + IMG_PATH + "/" + imgFileName, function (err) {
      if(err) {
        ...
        reqImgFile.mv('./' + AUDIO_PATH + "/" + audFileName, function (err) {
          if(err) {
            res.send(err);
          } else {
            res.send("Your files have been added to the server and you may now go back to the previous screen, thank you,!");
          }
      });
      }
    });
